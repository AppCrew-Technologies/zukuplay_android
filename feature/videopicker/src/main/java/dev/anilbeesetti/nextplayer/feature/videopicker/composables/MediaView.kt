...
// Ad UI is injected from the app module; keep this module ad-agnostic
...
// Manual dependency injection
    ...
    val adRepository = AdRepository(firestore)
    val adMobManager = AdMobManager()
    adManager = AdManager(adRepository, adMobManager)
    if (isLoading) {
        ...
        MediaLazyList {
            ...
            itemsIndexed(rootFolder.mediaList, key = { _, video -> video.path }) { index, video ->
                LaunchedEffect(Unit) {
                ...
                // ✅ Insert Ad Banner after every 8th video
                if ((index + 1) % 8 == 0) {
                    Spacer(modifier = Modifier.height(8.dp))
                    if(AdConstants.SHOW_AD_BETWEEN_VIDEO_CLIPS_LIST){
                        BannerAdViewNew()
                    }
